...
from oauth.external_urls import instagram_api_url, instagram_authorization_base_url, instagram_redirect_uri, instagram_token_url, frontend_channel_url
...
instagram_scopes = ['ads_read', 'ads_management', 'public_profile', 'email', 'pages_show_list', 'pages_read_engagement', 'pages_read_user_content']
instagram = OAuth2Session(instagram_client_id, redirect_uri=instagram_redirect_uri, scope=instagram_scopes)
instagram = instagram_compliance_fix(instagram)
...
def instagram_oauth(request):
    ...
    try:
        authorization_url = f"{instagram_authorization_base_url}/?client_id={instagram_client_id}&redirect_uri={instagram_redirect_uri}&scope=user_profile,user_media&state={state_encoded}&response_type=code"
...
def instagram_oauth_callback(request):   
    try:
        ...
        token = instagram.fetch_token(token_url=instagram_token_url, client_secret=instagram_client_secret, # 60days validity
                                     authorization_response=redirect_response) # access_token

        access_token = token.get("access_token")
        instagram_data = get_instagram_data(access_token)

        try:
            instagram_channel = get_channel(email=user_email, channel_type_num=2)
        except Exception:
            instagram_channel = create_channel(email=user_email, channel_type_num=2)

        if instagram_channel.credentials is None:
            credentials = APICredentials.objects.create(
                key_1=access_token,
                key_2=instagram_data['ad_account_ids']
            )
            instagram_channel.credentials = credentials
        else:
            instagram_channel.credentials.key_1 = access_token
            instagram_channel.credentials.key_2 = instagram_data['ad_account_ids']
...
def get_instagram_data(access_token):
    user_url = '{instagram_api_url}/me'
    ads_accounts_url = '{instagram_api_url}/me/adaccounts'
...
def get_instagram_campaign_data(access_token, account_list):
    campaign_account_list = []
    for account in account_list:
        campaign_url = f"{instagram_api_url}/{account}/campaigns"
        campaign_params = {
            "access_token": access_token,
...
def get_instagram_campaign_statistics(access_token, campaign_data):
    for account_campaings in campaign_data:
        for campaigns in account_campaings:
            ...
            campaign_insights_url = f"{instagram_api_url}/{campaigns['id']}/insights"
...
def get_page_posts(access_token):
    ...
    pages_url = f'{instagram_api_url}/me/accounts'
    ...
    for page in pages_data:
        ...
        posts_url = f'{instagram_api_url}/{page_id}/posts'
    ...
    for account_campaings in campaign_data:
        ...
        for campaings in account_campaings:
            ...
            for adsets in adsets_list:
                ad_set_url = f"{instagram_api_url}/{adsets['id']}/ads"
                ...
                for ads in ads_data:
                    creative_url = f"{instagram_api_url}/{ads['creative']['id']}"
                    creative_params = {
                        'access_token' : access_token,
...
def get_adset_data(access_token, campaing_data):
    for account_campaings in campaing_data:    
        for campaings in account_campaings:
            ...
            adsets_data = []
            for adsets in adsets_list:
                ad_set_url = f"{instagram_api_url}/{adsets['id']}"
...
def get_instagram_marketing_data(access_token, ad_account_list):
    marketing_data = []
    
    try:    
        campaign_data = get_instagram_campaign_data(access_token, ad_account_list)
        campaing_data = get_adset_data(access_token, campaign_data)
        campaign_data = get_ads_creative_data(access_token,campaing_data)
        campaign_data = get_instagram_campaign_statistics(access_token, campaign_data)
